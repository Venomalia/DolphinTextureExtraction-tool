...
namespace LibCPK
{
    public class CriCompression
    {
        public static unsafe int CRIcompress(byte* dest, int* destLen, byte* src, int srcLen)
        {
            int n = srcLen - 1, m = *destLen - 0x1, T = 0, d = 0, p, q = 0, i, j, k;
            byte* odest = dest;
            for (; n >= 0x100;)
            {
                j = n + 3 + 0x2000;
